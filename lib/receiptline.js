...
(function () {
    ...
    function parseOption(printer) {
        ...
        return {
            cpl: p.cpl || 48,
            encoding: supportedEncodings.includes(p.encoding)
                ? p.encoding
                : 'cp437',
            ...
            .map((column, index, array) => {
                ...
                // parse text
                    ...
                    result.text = element
                        ...
                        .replace(
                            /[\x00-\x1f\x7f]|\\x[01][\dA-Fa-f]|\\x7[Ff]/g,
                            ''
                        )
                ...
                if (state.width.length === 0) {
                    ...
                    result.width =
                        index < state.width.length ? state.width[index] : 0;
            ...
            for (let j = 0; j < row; j++) {
                ...
                if (state.line === 'running') {
                    ...
                    const height = cols.reduce(
                        (a, col) =>
                            j < col.length ? Math.max(a, col[j].height) : a,
                        1
                    );
    ...
    const _base = {
        ...
        measureText: (text, encoding) => {
            ...
            switch (encoding) {
                ...
                case 'big5':
                    r = t.reduce(
                        (a, c) => a + (c.codePointAt(0) < 0x80 ? 1 : 2),
                        0
                    );
                    break;
                case 'tis620':
    ...
    const _svg = {
        ...
        open: function (printer) {
            ...
            this.feedMinimum = Number(
                this.charWidth * (printer.spacing ? 2.5 : 2)
            );
        ...
        text: function (text, encoding) {
            ...
            this.textPosition +=
                this.measureText(text, encoding) * this.textScale;
            return '';
        },
        // feed new line:
        lf: function () {
        ...
        image: function (image, align, left, width, right) {
            ...
            const margin =
                this.lineMargin * this.charWidth +
                ((this.lineWidth * this.charWidth - imgWidth) *
                    this.lineAlign) /
                    2;
            ...
            if (typeof qrcode !== 'undefined') {
                ...
                qr.createSvgTag(symbol.cell, 0).replace(
                    ...
                    (match, w, h, path) => {
                        const margin =
                            this.lineMargin * this.charWidth +
                            ((this.lineWidth * this.charWidth - Number(w)) *
                                this.lineAlign) /
                                2;
        ...
        barcode: function (symbol, encoding) {
            ...
            if ('module' in bar) {
                ...
                const margin =
                    this.lineMargin * this.charWidth +
                    ((this.lineWidth * this.charWidth - width) *
                        this.lineAlign) /
                        2;
                this.svgContent += `<g transform="translate(${margin},${this.svgHeight})">${path}</g>`;
        ...
        code128: function (data) {
            ...
            if (s.length > 0) {
                ...
                // generate modules
                r.module =
                    '0' + d.reduce((a, c) => a + this.c128.element[c], '');
        ...
        code128a: function (x, s, d) {
            ...
            s = s.replace(
                ...
                m => (
                    m
                        .split('')
                        .forEach(c => d.push((c.charCodeAt(0) + 64) % 96)),
                    ''
                )
            );
            s = s.replace(
        ...
        code128b: function (x, s, d) {
            ...
            s = s.replace(
                /^((?!\d{4,})[ -\x7f])+/,
                m => (
                    m.split('').forEach(c => d.push(c.charCodeAt(0) - 32)), ''
                )
            );
            s = s.replace(
                /^\d(?=\d{4}(\d{2})*)/,
        ...
        code93: function (data) {
            ...
            if (s.length > 0) {
                ...
                d.push(
                    d.reduceRight(
                        (a, c, i) => a + c * (((d.length - 1 - i) % 20) + 1)
                    ) % 47
                );
                d.push(
                    d.reduceRight(
                        (a, c, i) => a + c * (((d.length - 1 - i) % 15) + 1)
                    ) % 47
                );
                ...
                // generate modules
                r.module =
                    '0' + d.reduce((a, c) => a + this.c93.element[c], '');
            ...
            if (s.length > 0) {
                ...
                r.module =
                    ...
                    s
                        .toUpperCase()
                        .split('')
                        .reduce((a, c) => a + this.nw7[c] + '2', '')
                        .slice(0, -1);
                r.length =
                    s.length * 25 -
                    ((s + '$').match(/[\d\-$]/g).length - 1) * 3 -
                    2;
            }
            return r;
        },
    ...
    const _thermal = {
        ...
        vrlf: function (vr) {
            return (
                (vr === this.upsideDown && this.spacing
                    ? '\x1b2'
                    : '\x1b3\x00') + this.lf()
            );
        },
                    ...
                    return (
                        ...
                        this.arrayFrom(text, encoding).reduce(
                            (a, c) =>
                                a +
                                '\x00' +
                                iconv.encode(c, encoding).toString('binary'),
                            ''
                        )
                    );
                default:
        ...
        qrcode: function (symbol, encoding) {
            const d = iconv
                .encode(
                    symbol.data,
                    encoding === 'multilingual' ? 'ascii' : encoding
                )
                .toString('binary')
                .slice(0, 7089);
            return (
    ...
    const _star = {
        ...
        close: function () {
            return (
                (this.cutting ? this.cut() : '') + '\x1b\x1d\x03\x01\x00\x00'
            );
        },
        ...
        image: function (image, align, left, width, right) {
            ...
            return (
                this.area(left, width, right) + this.align(align) + s.join('')
            );
        },
    ...
    const _stargraphic = {
        ...
        lf: function () {
            return (
                '\x1b*rY' + this.charWidth * (this.spacing ? 2.5 : 2) + '\x00'
            );
        },
        // insert commands:
        command: command => command,
